Начало:
nmap -sV --open -oA nibbles_initial_scan 10.129.200.170 (-oA nibbles_initial_scan сохраняет результаты сканирования в файлы. "-oA" значит "output in All formats" — создаст три файл. nibbles_initial_scan - имя файла)
nmap -v -oG - 10.129.200.170 (выводит формат, допускающий grep, в стандартный вывод)
nmap -p- --open -oA nibbles_full_tcp_scan 10.129.42.190 (проверим наличие служб)
...
nc -nv 10.129.42.190 22 (22 (ssh) open SSH-2.0-OpenSSH_7.2p2 Ubuntu-4ubuntu2.8)
...
Firefoxhttp://nibbleblog/content/private/plugins/my_image/image.php, чтобы запустить обратную оболочку (после загрузки php скрипта на серв)
